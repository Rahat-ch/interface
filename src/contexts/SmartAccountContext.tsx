...
const supportedChains = [ChainId.GOERLI, ChainId.POLYGON_MAINNET, ChainId.POLYGON_MUMBAI]
  ...
  const getSmartAccount = useCallback(async () => {
    ...
    try {
      ...
      smartAccount.on('txHashChanged', (response: any) => {
        ...
        console.log(`Transaction updated with hash: ${response.hash}`, response.hash)
      })

      smartAccount.on('txMined', (response: any) => {
        console.log('txMined event received in AddLP via emitter', response)
        console.log(`Transaction confirmed: ${response.hash}`, response.hash)
      })
